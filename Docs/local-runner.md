...
Both install it at `~/.openfusion/bin/fusion-runner`, create a
`~/.local/bin/fusion-runner` symlink, write `~/.openfusion/config.json`, and
...
install the runner under `%USERPROFILE%\.openfusion\bin`, create a
`fusion-runner.cmd` shim, add the shim directory to the user PATH, write
`%USERPROFILE%\.openfusion\config.json`, and register the
...
The hosted browser UI cannot directly spawn a local binary or scan the user's PATH. OpenDesign appears native because its Electron package starts a privileged daemon sidecar that performs local agent detection and CLI spawning. openFusion uses the same trust boundary through the Go runner.
