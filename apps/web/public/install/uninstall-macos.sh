...
if [[ -n "$resolved_config" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    runner_id="$(python3 -c "import json; d=json.load(open('$resolved_config')); print(d.get('runner_id') or '')" 2>/dev/null || true)"
    config_cloud_url="$(python3 -c "import json; d=json.load(open('$resolved_config')); print(d.get('cloud_url') or '')" 2>/dev/null || true)"
    config_token="$(python3 -c "import json; d=json.load(open('$resolved_config')); print(d.get('token') or '')" 2>/dev/null || true)"
  else
...
if [[ -z "$cloud_url" ]]; then
  cloud_url="${config_cloud_url:-$default_cloud_url}"
fi
if [[ -z "$token" ]]; then
  token="$config_token"
fi
...
if [[ "$skip_api" -eq 0 && -n "$runner_id" ]]; then
  if [[ -z "$token" ]]; then
    ...
    encoded_runner_id="$runner_id"
    if command -v python3 >/dev/null 2>&1; then
      encoded_runner_id="$(python3 -c 'import sys,urllib.parse; print(urllib.parse.quote(sys.argv[1], safe=""))' "$runner_id" 2>/dev/null || printf '%s' "$runner_id")"
    fi
    echo "Deregistering runner $runner_id from $cloud_url ..."
    status="$(curl -sS -o /dev/null -w '%{http_code}' \
      -X DELETE \
      -H "authorization: Bearer $token" \
      "${cloud_url%/}/api/runners/$encoded_runner_id" \
      2>/dev/null || true)"
    if [[ "$status" == "200" ]]; then
